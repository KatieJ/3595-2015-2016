...
public class TeleOp9 extends OpMode
{
    ...
    double liftDeadband = 0.1; // Deadband for trigger which moves lift
    // Servo positions
...
/*
        if (gamepad1.left_trigger != 0.0)
            telemetry.addData("LEFT TRIGGER NOT ZERO: ", gamepad1.left_trigger);
        else
            telemetry.addData("left trigger", "good");
        if (gamepad1.right_trigger != 0.0)
            telemetry.addData("RIGHT TRIGGER NOT ZERO: ", gamepad1.right_trigger);
        else
            telemetry.addData("right trigger", "good");
...
*/
    ...
    public void loop () // Main tele-op code
    {
        ...
        if(!leftDebrisScore && !rightDebrisScore) {
        ...
        if (gamepad1.right_bumper) // Raise lift while pressed
        {
            liftMotor1.setPower(1.0);
            liftMotor2.setPower(1.0);
        }
        else if (rightTrigger >= liftDeadband) // Lower lift while pressed more than deadband
        {
        ...
        // All brush code is convoluted and not yet properly commented -- feel free to skip it
        ...
        */
        ...
        yButton = gamepad1.y; // For less frequent retrieval
        lockBrake = updateButton(yButton, lastY, lockBrake); // Move servo?
        lastY = yButton; // Update last value
        // Update brake servo
        if (lockBrake)
    ...
    String getPercentage(double currentPos) { // Get percentage that lift is extended
        ...
        return Integer.toString(percentage); // Return as a string for ease of reading
    }
