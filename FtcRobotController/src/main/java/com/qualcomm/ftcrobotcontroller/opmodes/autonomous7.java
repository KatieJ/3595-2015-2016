// Most clicks are now off
// If i need to revert to a working program use this, delete the absolute value stuff in gyro methods, and uncomment all of the gyro correction backward
...
public class autonomous7 extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        gyroCorrectionForward(-0.35, 2250, 2);
        ...
        gyroCorrectionForward(-0.3, 11400, 5.5);
        // start slowly so that it doesn't pop a wheelie maybe
        ...
        gyroCorrectionForward(0.3, 700, 1.5);

        turnClockwise(.65, 46);

        //gyroCorrectionBackward(-0.4, 16000, 3.0);
        gyroCorrectionForward(-0.4, 16000, 3.0);

        while (autoTimer.time() < 26) {
            telemetry.addData("while loop running", i);
            sleep(50);
        }
        gyroCorrectionForward(1.0, 2500, 4.0); // clicks are so big because I want it to time out instead of going for distance (because I'm pushing another robot probably)
    }
            ...
            throws InterruptedException
    {
        ...
        // absolute value might cause problems if number is negative and rolling over to positive ever
        while (absoluteValue(leftDrive1.getCurrentPosition()) < clicks && driveTimer.time() < timeout) {
            telemetry.addData("Encoder counts: ", leftDrive1.getCurrentPosition());
