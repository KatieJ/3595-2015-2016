...
// One of 8 autonomous programs used at Supers

public class BlueAutoNearMountTest extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        gyroCorrection(-0.35, 3000, 4.0);

        // Turn towards shelter
        turnClockwise(0.75, 46);

        // Drive to shelter
        gyroCorrection(-0.35, 8000, 5.5);
        ...
        // Retract servos
        leftAutoServo.setPosition(leftAutoIn);
        sleep(1000); // Wait for servo to retract (play with this time)

        // Back away from shelter (so that robot doesn't catch when turning)
        gyroCorrection(0.3, 2000, 1.5);

        turnCounterClockwise(.65, 46);

        leftBrushMotor.setPower(-1.0);
        rightBrushMotor.setPower(-1.0);
        gyroCorrection(-0.4, 6000, 5.0);
        leftBrushMotor.setPower(0.0);
        rightBrushMotor.setPower(0.0);

        // Wait for final 4 seconds of autonomous
        while (autoTimer.time() < 26) {
            sleep(50);
        }
        // Drive back into beacon repair zone
            ...
            throws InterruptedException
    {
        ...
        int backupTime = 200;
        double xDegreesTurned = 0.0; // Degrees robot has titlted
        double error;
        ...
        while (absoluteValue(leftDrive1.getCurrentPosition()-zeroedEncoders) < clicks
            ...
            waitOneFullHardwareCycle(); // Deprecated
            currentTime = driveTimer.time();
            loopSpeed = currentTime - previousTime;
            previousTime = currentTime;

            // Gyro returns degrees per second, multiply by loop speed to solve for degrees
            ...
            if (xDegreesTurned > 250) { // If robot is on debris, back up
                ...
                leftMotorPower = motorPower - error * gain;
                rightMotorPower = motorPower + error * gain;
            }
    ...
    // Needed an absolute value method, standard one wasn't supported so we wrote our own
