...
public class TeleOpBlue2 extends OpMode
{
    ...
    {
        telemetry.addData("Encoder lower limit?", liftLowerLimit);
        ...
        liftLowerLimit = updateButton(gamepad1.back, lastBack, liftLowerLimit);
        rightTrigger = gamepad1.right_trigger; // For less frequent retrieval
        // The if statement on the following line is ugly code, needed a quick way to do this
        // a couple days before worlds
        if (liftLowerLimit) {
        ...
        */
        ...
        if((leftDebrisScore || rightDebrisScore) && !fendersDown) {
            leftDrive1.setPower(Range.clip((leftPower+0.1), -1.0, 1.0));
